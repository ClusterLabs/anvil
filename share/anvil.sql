...
-- use 'host_uuid'.
-- 
-- All tables must hast a 'change_date  timestamp with time zone  not null' column. This is used to track
...
CREATE TABLE hosts (
    ...
    host_type        text                        not null,                   -- Either 'node', 'dashboard' or 'dr'.
...
--          history_host_variable.change_date);
--     RETURN NULL;
-- END;
-- $$
-- LANGUAGE plpgsql;
...
--          history_network_interfaces.change_date);
--     RETURN NULL;
-- END;
-- $$
-- LANGUAGE plpgsql;
...
--          history_ip_addresses.change_date);
--     RETURN NULL;
-- END;
-- $$
-- LANGUAGE plpgsql;
...
-- 'change_date' changes per run, so that database resyncs can be triggered reliably.
